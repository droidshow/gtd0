...
** CMC TASKS
*** WAITING Verify with Binduji if this is all the classes            :GMAIL:
  SCHEDULED: <2017-07-04 Tue>
 Prahald 
 Markandeya
 Dhruva
 Luv Kush
...
*** DONE Host family for Swamini 
    CLOSED: [2017-07-03 Mon 15:35]
*** STARTED Give feedback on registration
    SCHEDULED: <2017-06-28 Wed>

Browser: Firefox 54

The bacground picture is only partially visible upto face.
https not enabled
Clicked on signup
Entered name
Entered mail
Got a pop-up saying password has been sent to mail id
Password received
The mail says contact CM Registration team if there are problems, the mail id is columbus@chinmayamission.org, is the registration monitioring this ?
A link can be provided with the mail to loging page
Tried wrong maild id, got a pop saying user unknown
After login shows Madhus contact (mail and phone)
Perhaps ask user to change password after first login ?
Could change password succesfull, logged out and asked to login againg with new password
Fields like DoB and Phone need not be mandatory
Could add 2 profiles, forces to add fields for others
Clicked on Enroll one of the profile, did fancy rotate to open a new frame but the background text flipped.
Register for drop down list
 - should be better sorted
 - should be better worded
Language class timings shows up as 0s
Sevak Satsangh day of the week not mentioned
Day of the week can be mentioend for Sunday offerings also
Acceptance waiver language needs correction.
...
In payment history section:
 Academic year ends on 1970-01-01 is incorrect
